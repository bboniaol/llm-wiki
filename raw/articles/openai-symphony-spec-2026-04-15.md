...
- Original URL: https://raw.githubusercontent.com/openai/symphony/main/SPEC.md
- Retrieved on: 2026-04-15
- Publisher: OpenAI
- Type: GitHub spec / source document
- Main claim: Symphony 是一个 long-running automation service，用于把 issue tracker 中的 project work 转成 isolated autonomous coding-agent runs；其关键是 per-issue workspace、repo-owned WORKFLOW.md、bounded concurrency、restart recovery 与 observability。
- Key signals:
  - scheduler/runner + tracker reader，而不是通用 workflow engine
  - policy in repo via WORKFLOW.md
  - deterministic per-issue workspaces, backoff, reconciliation, restart recovery
  - success state 可以停在 Human Review，而不一定直接 Done
