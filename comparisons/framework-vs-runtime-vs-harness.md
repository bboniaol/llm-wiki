...
sources: [raw/articles/langchain-anatomy-of-an-agent-harness-2026-04-14.md, raw/articles/langchain-deepagents-overview-2026-04-14.md, raw/articles/inngest-harness-not-framework-2026-04-14.md, raw/articles/lijiuer-harness-electricity-analogy-2026-04-09.md]
---
...
## 比喻视角的补充
[[lijiuer-harness-electricity-analogy-2026-04-09|李韭二]] 的"LLM 是电"文章从心智模型角度对三层做了另一种拆解，用比喻来检验每层的本质：

| 比喻 | Framework | Runtime | Harness |
|------|-----------|---------|---------|
| **零件/发动机/车**（现有） | 零件箱 | 发动机和底盘 | 能上路、带护栏和仪表盘的车 |
| **电力产业**（李韭二补充） | 电器设计规范 | 输电网 | **电器** — 将标准化电力转化为特定场景价值的完整工程系统 |
| **计算机**（Beren Millidge） | 编程语言/编译器 | 操作系统内核 | **操作系统** — 让裸 CPU 可用的完整系统 |
| **托管模式**（Claude Managed Agents） | 模板配置（YAML） | 托管基础设施 | **托管 harness** — brain/hands/session 三体分离，基础设施即 harness |

关键洞察：
- **Framework 不是 harness**：就像电气工程规范不等于电器本身。LangChain 提供 building blocks，但 Claude Code / Codex / Manus 才是能跑的 harness。
- **Harness 不会消失**：就像电力普及后电器没有消失，反而越来越多。模型增强会让 harness"变薄"（移除一些旧复杂度），但不会让 harness 本身消失。
- **Harness Engineering 是学科**：不是"套壳"，而是"电气工程"——制造电器的完整工程学科。
...
- [[lance-martin-claude-managed-agents-2026-04-09|lance-martin-claude-managed-agents]] — 托管 harness 的实例化
