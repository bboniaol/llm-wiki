...
### 11. **Initializer → Coding Agent 接力模式（Akshay 综合）**
Akshay Pachaar 把 Anthropic 的 initializer/coding agent 接力收束成更通用的 harness 生命周期管理：Initializer Agent 铺环境（init script、progress file、feature list、initial git commit），后续 Coding Agent 每轮读 git log + progress file orient 自己、选最高优先级未完成 feature、工作、commit、写 summary。filesystem 提供跨 context window 的连续性。这与 Lychee 的 7 层 harness 示例形成互补：Lychee 展示的是单轮内各层配合，Akshay 展示的是跨轮次生命周期管理。
...
- Akshay Pachaar 的 7-step TAO cycle（Prompt Assembly → LLM Inference → Output Classification → Tool Execution → Result Packaging → Context Update → Loop）可以作为所有 coding agent workflow 的通用骨架。无论具体工具是什么，这 7 步都适用。
