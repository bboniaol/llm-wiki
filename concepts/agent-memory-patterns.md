...
- **Akshay Pachaar 的三层记忆模型**：Claude Code 实现了一个很清晰的三层记忆层级——(1) lightweight index（~150 字符/条目，始终加载）→ (2) detailed topic files（按需拉取）→ (3) raw transcripts（仅通过搜索访问）。关键设计原则：agent 把自己的记忆当作 "hint"，行动前必须对照实际状态验证。这与 "trust but verify" 的可靠性原则直接兼容。
- **OpenAI 的四类持久策略**：application memory、SDK sessions、server-side Conversations API、lightweight previous_response_id chaining。四类互斥，选择哪一类直接决定了 state 的 ownership 边界。
- **LangGraph 的 namespace JSON Stores**：长期记忆按 namespace 组织，支持跨会话检索。与 Claude Code 的文件系统记忆形成两种路线对比：结构化存储 vs 文件型记忆。
- **记忆作为 hint 而非 truth**：Akshay 强调 agent 不应盲目信任自己的记忆文件，而应在每次行动前重新验证。这与 [[agent-reliability-patterns]] 中的 "fail locally" 原则一致：记忆偏差应在单步内被发现和修正，而不是累积成系统性幻觉。
